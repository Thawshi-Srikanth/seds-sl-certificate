...
export const ParticipantsManager: React.FC<ParticipantsManagerProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Search & Actions Bar */}
      <div className="apple-card space-y-3 rounded-2xl p-4">
        <div className="flex flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="apple-input w-full rounded-xl py-2 pl-9 pr-3 text-xs placeholder-zinc-500 sm:text-sm"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-zinc-700 bg-zinc-800 px-3 py-2 text-xs font-medium text-zinc-200 transition-colors hover:bg-zinc-700"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl bg-white px-3 py-2 text-xs font-semibold text-black transition-colors hover:bg-zinc-200"
            >
        ...
        {/* Filters */}
        <div className="flex items-center gap-1.5 overflow-x-auto border-t border-zinc-800/80 pt-1 text-xs">
          {(['all', 'eligible', 'ineligible', 'claimed', 'unclaimed'] as const).map((key) => (
            <button
              ...
              className={`rounded-lg px-2.5 py-1 text-xs font-medium capitalize transition-colors ${
                filter === key ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:text-zinc-200'
              }`}
            >
              {key}
            </button>
          ...
          >
            <RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin text-zinc-300' : ''}`} />
          </button>
        </div>
      </div>
        ...
        <div
          className={`flex items-center justify-between rounded-xl p-3 text-xs ${
            actionFeedback.type === 'success'
              ? 'border border-emerald-500/20 bg-emerald-950/20 text-emerald-300'
      ...
      {importResult && (
        <div className="flex items-center justify-between rounded-xl border border-emerald-500/20 bg-emerald-950/20 p-3 text-xs text-emerald-300">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-400" />
            <span>
      ...
      {previewRows && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="apple-card flex max-h-[80vh] w-full max-w-2xl flex-col space-y-4 rounded-2xl p-6">
            ...
            <div className="flex-1 overflow-auto rounded-xl border border-zinc-800">
              <table className="w-full text-left text-xs text-zinc-300">
                <thead className="sticky top-0 bg-zinc-900 font-medium uppercase text-zinc-400">
                  <tr>
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              <button
                type="button"
                onClick={() => setPreviewRows(null)}
                className="rounded-xl border border-zinc-800 px-3 py-1.5 text-xs text-zinc-300 hover:text-white"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-xl bg-white px-4 py-1.5 text-xs font-semibold text-black hover:bg-zinc-200 disabled:opacity-50"
              >
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="py-10 text-center text-zinc-500">
                    <Loader2 className="mx-auto mb-1.5 h-5 w-5 animate-spin text-zinc-400" />
                    <span>Loading...</span>
                  </td>
                </tr>
                  ...
                  <tr key={p.id} className="hover:bg-zinc-900/30">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        ...
                        className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium transition-colors ${
                          p.eligible
                            ? 'border border-emerald-500/20 bg-emerald-950/20 text-emerald-300 hover:bg-emerald-900/30'
                    ...
                    <td className="px-4 py-3 text-xs">
                      {p.certificate_claimed ? (
                        <div>
                          <span className="font-medium text-emerald-300">Claimed</span>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="apple-card w-full max-w-sm space-y-4 rounded-2xl p-6">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
            ...
            {modalError && (
              <div className="rounded-xl border border-rose-500/20 bg-rose-950/20 p-2.5 text-xs text-rose-300">
                {modalError}
              </div>
            )}
            ...
            <form onSubmit={handleAddParticipant} className="space-y-3">
              <div>
                <label className="mb-1 block text-xs text-zinc-300">Full Name</label>
                <input
                  type="text"
                  required
                  value={newName}
              ...
              <div>
                <label className="mb-1 block text-xs text-zinc-300">Registration ID</label>
                <input
                  type="text"
                  value={newRegId}
              ...
              <div className="flex items-center justify-between rounded-xl border border-zinc-800 p-3">
                <span className="text-xs text-zinc-300">Eligible for Certificate</span>
                <input
                  type="checkbox"
                  checked={newEligible}
                  onChange={(e) => setNewEligible(e.target.checked)}
                  className="h-4 w-4 rounded border-zinc-700 bg-zinc-800 text-white"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowAddModal(false)}
                  className="rounded-xl border border-zinc-800 px-3 py-1.5 text-xs text-zinc-300"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="rounded-xl bg-white px-4 py-1.5 text-xs font-semibold text-black hover:bg-zinc-200"
                >
